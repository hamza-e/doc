...
@section('footer')
    @if(Auth::user()->role == 'medecin')
        ...
        var datefin = "{{ date('Y-m-d',strtotime(Auth::user()->active_a)) }}";
        var daysLeft = moment(datefin).diff(moment(),'days') + 1;
        console.log(daysLeft);
        if(daysLeft < 15 && daysLeft > 0)
            alert('Il vous reste '+daysLeft+' jours Veuillez Renouveler votre pack!');
        else if( daysLeft < 0)
            alert('Pack épuisé!! Veuillez Renouveler votre pack!');
        </script>
    @endif
@endsection